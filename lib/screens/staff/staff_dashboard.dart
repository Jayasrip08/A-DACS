...
class _StaffDashboardState extends State<StaffDashboard> {
  ...
  String _loadingMessage = "Initializing department...";
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      for (var doc in studentSnapshot.docs) {
        ...
        if (!activeBatchNames.contains(batch.toLowerCase().trim())) continue; 
        ...
        if (mainDeadline != null && DateTime.now().isAfter(mainDeadline)) {
           if (verifiedMain < mainFee) isOverdue = true;
        }
        if (!isOverdue && examDeadline != null && DateTime.now().isAfter(examDeadline)) {
           if (verifiedExam < examFee) isOverdue = true;
        }
      ...
      for (var s in processedList) {
        ...
        if (regNo.isEmpty) { uniqueStudents[s['uid']] = s; continue; }
        if (uniqueStudents.containsKey(regNo)) {
          final existing = uniqueStudents[regNo]!;
          if ((s['verifiedPaid'] + s['pendingPaid']) > (existing['verifiedPaid'] + existing['pendingPaid'])) uniqueStudents[regNo] = s;
        } else {
      ...
      if (mounted) {
        setState(() => _isLoading = false);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Sync Error: $e"), backgroundColor: AppColors.red));
      }
    }
  }
  ...
  Map<String, dynamic> _calculateFeeDetails(List<Map<String, dynamic>> structures, String batch, String dept, String type, String quota, String busPlace) {
    ...
    for (var struct in structures) {
      ...
      if (struct['deadline'] != null) {
        ...
        if (dt != null && (earliestDeadline == null || dt.isBefore(earliestDeadline))) earliestDeadline = dt;
      }
      if (struct['examDeadline'] != null) examDeadline = (struct['examDeadline'] as Timestamp?)?.toDate();
    ...
    return { 'amount': total, 'mainFee': total - examFee, 'examFee': examFee, 'deadline': earliestDeadline, 'examDeadline': examDeadline };
  }
  ...
  Future<void> _downloadReport() async {
    ...
    if (targetList.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("No records available to report")));
      return;
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: _isSelectionMode 
          ...
          : null,
        title: Text(
          _isSelectionMode ? "${_selectedIds.length} Selected" : (_staffDept != null ? "$_staffDept Operations" : "Staff Console"),
        ),
        actions: [
          if (!_isSelectionMode)
            IconButton(
              onPressed: _downloadReport, 
              icon: const Icon(Icons.cloud_download_outlined, color: AppColors.red),
        ...
        children: [
          Column(
            children: [
              ...
              Expanded(
                child: _isLoading 
                ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [const CircularProgressIndicator(), const SizedBox(height: 16), Text(_loadingMessage, style: const TextStyle(color: AppColors.slateLight))]))
                : filteredList.isEmpty
                    ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.person_search_rounded, size: 64, color: AppColors.slateLighter), const SizedBox(height: 16), const Text("No matching records found.", style: TextStyle(color: AppColors.slateLight, fontWeight: FontWeight.bold))]))
                    : ListView.builder(
      ...
      bottomNavigationBar: Container(
        ...
        child: BottomNavigationBar(
          ...
          items: const [
            BottomNavigationBarItem(icon: Icon(Icons.dashboard_outlined), activeIcon: Icon(Icons.dashboard_rounded), label: 'Monitor'),
  ...
  Widget _buildHeader(int count) {
    return Container(
      ...
      child: Text(
        "CURRENTLY TRACKING $count STUDENTS",
        style: GoogleFonts.outfit(color: Colors.white.withOpacity(0.8), fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.5),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Container(
            ...
            child: TextField(
              decoration: const InputDecoration(
                ...
                prefixIcon: Icon(Icons.search_rounded, color: AppColors.red),
          ...
          Row(
            children: [
              Expanded(child: _buildCustomDropdown(_selectedBatch, "Select Batch", _batches, (v) => setState(() => _selectedBatch = v))),
              const SizedBox(width: 12),
              Expanded(child: _buildCustomDropdown(_selectedStatus, "Fee Status", ['All', 'Verified', 'Overdue', 'Not Paid'], (v) => setState(() => _selectedStatus = v!))),
            ],
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: val == 'All' && hint.contains("Status") ? 'All' : val,
          hint: Text(hint, style: const TextStyle(fontSize: 13, color: AppColors.slateLight)),
          isExpanded: true,
          icon: const Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.grey),
          items: items.map((e) => DropdownMenuItem(value: e, child: Text(e, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppColors.slate)))).toList(),
          onChanged: onChange,
        ),
      ),
    );
  ...
  Widget _buildStudentCard(Map<String, dynamic> item) {
    ...
    if (isNoFee) { statusColor = AppColors.grey; statusText = "Config Required"; }
    else if (isPaid) { statusColor = AppColors.success; statusText = "Verified Cleared"; }
    else if (isPending) { statusColor = AppColors.info; statusText = "In Verification"; }
    else if (isOverdue) { statusColor = AppColors.red; statusText = "Payment Overdue"; }
    else { statusColor = AppColors.slate; statusText = "Dues Outstanding"; }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: isSelected ? Border.all(color: AppColors.red, width: 2) : null,
        boxShadow: AppColors.softShadow,
      ),
      child: ListTile(
        onTap: () {
        ...
        leading: Container(
          ...
          height: 50,
          decoration: BoxDecoration(
            color: isSelected ? AppColors.red : statusColor.withOpacity(0.08),
            borderRadius: BorderRadius.circular(16),
          ),
          child: Center(
            child: isSelected 
              ? const Icon(Icons.check_rounded, color: Colors.white) 
              : Text(item['name'][0].toUpperCase(), style: TextStyle(color: statusColor, fontWeight: FontWeight.bold, fontSize: 20)),
          ),
        ),
        title: Text(item['name'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: AppColors.slate)),
        subtitle: Padding(
          padding: const EdgeInsets.only(top: 4),
          child: Text("${item['regNo']} • ${item['batch']}", style: const TextStyle(color: AppColors.slateLight, fontSize: 13, fontWeight: FontWeight.w500)),
        ),
        trailing: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(statusText.toUpperCase(), style: TextStyle(color: statusColor, fontWeight: FontWeight.w900, fontSize: 9, letterSpacing: 0.5)),
            const SizedBox(height: 6),
            Text("₹${balance.toStringAsFixed(0)}", style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 18, color: AppColors.slate)),
          ],
        ),
      ),
    );
