...
class _RegisterScreenState extends State<RegisterScreen> {
    ...
    if (widget.verifiedStudentData != null) {
      ...
      if (widget.verifiedStudentData!.containsKey('type')) {
         ...
         if (['day_scholar', 'hosteller', 'bus_user'].contains(type)) {
            ...
            if (type == 'bus_user') _loadBusPlaces(); // pre-load routes
         }
      }
  ...
  Future<void> _loadBusPlaces() async {
    try {
      ...
      print("Error loading bus places: $e");
      if (mounted) {
        ErrorHandler.showError(context, 'Error loading bus routes: $e');
      }
    }
  }
                          ...
                          ? const Padding(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  SizedBox(width: 12),
                                  Text("Loading bus routes...", style: TextStyle(color: Colors.grey)),
                                ],
                              ),
                            )
                          : DropdownButtonFormField<String>(
