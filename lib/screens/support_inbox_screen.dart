...
class _SupportInboxScreenState extends State<SupportInboxScreen> {
    ...
    if (doc.exists) {
      if (mounted) {
        setState(() {
          ...
          _staffDept = doc.data()?['dept']?.toString().trim();
        });
      }
    }
  }
  ...
  Widget _buildTicketList({required bool active}) {
    ...
    // Filter by status (Only for Admin; Staff will filter in-memory to allow arrayContainsAny)
    if (_isAdmin) {
      if (active) {
    ...
    if (!_isAdmin) {
      if (_staffDept != null) {
        String dept = _staffDept!.toUpperCase();
        List<String> searchKeys = [_staffDept!, dept];
        
        // Add variations for common department names
        if (dept == 'IT' || dept == 'INFORMATION TECHNOLOGY') {
          searchKeys.addAll(['IT', 'Information Technology', 'IT Support', 'Technical']);
        } else if (dept == 'ACCOUNTS' || dept == 'FINANCE') {
          searchKeys.addAll(['Accounts', 'Finance', 'Accounts Office', 'Financial']);
        } else if (dept == 'ACADEMIC' || dept == 'EXAM') {
          searchKeys.addAll(['Academic', 'Exam', 'Academic Cell', 'Academic']);
        } else if (dept == 'ADMIN') {
          searchKeys.addAll(['Admin', 'General Admin', 'Office']);
        }

        query = query.where('visibility', arrayContainsAny: searchKeys);
      }
    }
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        // In-memory status filtering for Staff (because whereIn + arrayContainsAny is unsupported)
        if (!_isAdmin) {
          if (active) {
            docs = docs.where((d) => ['open', 'in_progress'].contains(d['status'])).toList();
          } else {
            docs = docs.where((d) => d['status'] == 'resolved').toList();
          }
        }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(data['description'] ?? "No message detail provided.", style: const TextStyle(height: 1.5)),
