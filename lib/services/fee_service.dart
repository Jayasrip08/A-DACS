...
class FeeService {
  ...
  Future<void> submitComponentProof({
    ...
    // 1. Human-centric Logic: Validate Transaction ID format
    if (transactionId.trim().length < 6) {
      throw Exception("Invalid Transaction ID. Please provide the full ID from your receipt.");
    }

    // 2. ID: uid_semester_feeType (Sanitized)
    ...
    final Map<String, dynamic> docData = {
      ...
      'amount': amountPaid + walletUsedAmount,
  ...
  Future<void> verifyPaymentComponent(String paymentId, bool isApproved, {String? rejectionReason}) async {
    await _db.runTransaction((transaction) async {
      ...
      if (studentId != null) {
        ...
        // Since transaction doesn't support external async calls like email sending,
        // we trigger it after the transaction logic block (this runs after transaction completes)
        final studentData = userSnapshot?.data() as Map<String, dynamic>?;
        if (studentData != null && studentData['email'] != null) {
          final studentEmail = studentData['email'];
          final studentName = studentData['name'] ?? 'Student';
          final semester = paymentData['semester'] ?? '?';
          ...
          if (isApproved) {
            ...
            EmailService().sendPaymentRejectedEmail(
              ...
              amount: amount,
              semester: semester,
              reason: rejectionReason ?? 'Please contact the accounts department for details.',
            );
          }
        }
      }
